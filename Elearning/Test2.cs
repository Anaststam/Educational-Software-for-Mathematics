...
namespace Elearning
{
    public partial class Test2 : Form
    {
        ...
        private void btnQuit_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            MySqlCommand cmd = con.CreateCommand();
            s = "Select * From elearningdb.results as rs Inner Join users as u on rs.userid=u.userid Where u.username='" + username + "' AND test2>=0;";
        ...
        private void FindQuestion(object sender, EventArgs e)
        {
            ...
            con.Open();
            s4 = "Select chap2weak from results where userid='" + userid + "' and chap2weak!='nullif()' ";
            ...
            if (mdr.Read())
            {
                weakness = mdr.GetString("chap2weak");
                if (weakness == "n")
                {
                    con.Close();
                    con.Open();
                    s = "select * from elearningdb.questions where kefalaio=2 order by rand() limit 4;";
                    mcd = new MySqlCommand(s, con);
                    mdr = mcd.ExecuteReader();
                }
                else
                {
                    con.Close();
                    con.Open();
                    s = "select * from elearningdb.questions where kefalaio=2 and difficulty='" + weakness + "' order by rand() limit 4;";
                    mcd = new MySqlCommand(s, con);
                    mdr = mcd.ExecuteReader();
                }
            }
