...
namespace Elearning
{
    public partial class Test3 : Form
    {
        ...
        private void btnQuit_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            string s2;
            s2 = "select userid,chap3weak from elearningdb.results where userid='" + userid + "' ";
            ...
            if (mdr.Read())
            {
                ...
                cmd6.CommandText = "Update results set chap3weak='" + weakness + "' where userid='" + userid + "' ;";
            ...
            else
            {
                ...
                cmd3.CommandText = "INSERT INTO results (userid,chap3weak) VALUES ('" + userid + "','" + weakness + "')  ";
            ...
            s1 = "select test3 from elearningdb.results where userid='" + userid + "' AND test3>=0 ;";
            mcd = new MySqlCommand(s1, con);
            mdr = mcd.ExecuteReader();

            if (mdr.Read())
            {
                string y = mdr.GetString("test3");
            ...
            con.Open();
            MySqlCommand cmd = con.CreateCommand();
            s = "Select * From elearningdb.results as rs Inner Join users as u on rs.userid=u.userid Where u.username='" + username + "' AND test3>=0;";
            mcd = new MySqlCommand(s, con);
            mdr = mcd.ExecuteReader();

            if (mdr.Read())
            {
                cmd.CommandText = "UPDATE results SET test3='" + (test1grade + (average * 100)) / 2 + "' where userid='" + userid + "'; ";
                Console.WriteLine(" είμαι εδώ 1! " + (test1grade + (average * 100)));

            }
            else
            {
                cmd.CommandText = "Update results SET test3 = '" + (average * 100) + "'where userid='" + userid + "';";
        ...
        private void FindQuestion(object sender, EventArgs e)
        {
            ...
            con.Open();
            s4 = "Select chap3weak from results where userid='" + userid + "' and chap3weak!='nullif()'";
            ...
            if (mdr.Read())
            {
                weakness1 = mdr.GetString("chap3weak");
                if (weakness1 == "n")
                {
                    con.Close();
                    con.Open();
                    s = "select * from elearningdb.questions where kefalaio=3 order by rand() limit 4;";
                    mcd = new MySqlCommand(s, con);
                    mdr = mcd.ExecuteReader();
                }
                else
                {
                    con.Close();
                    con.Open();
                    s = "select * from elearningdb.questions where kefalaio=3 and difficulty='" + weakness1 + "' order by rand() limit 4;";
                    mcd = new MySqlCommand(s, con);
                    mdr = mcd.ExecuteReader();
                }
            }
